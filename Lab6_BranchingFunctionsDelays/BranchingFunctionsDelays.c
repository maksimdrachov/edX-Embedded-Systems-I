...
int main(void){ unsigned long volatile delay;
  ...
  // initialization goes here
	...
	GPIO_PORTF_PCTL_R &= ~0x14;		// Clear the PF4 and PF2 bit fields in Port F PCTL to configure as GPIO
	GPIO_PORTF_DIR_R &= ~0x10;		// PF4 is an input
	GPIO_PORTF_DIR_R |= 0x04;		  // PF2 is an output
	GPIO_PORTF_AFSEL_R &= ~0x14;	// Clear the PF4 and PF2 bits in Port F AFSEL to disable alternate
  ...
  while(1){
    // body goes here
		unsigned long In;
		Delay100ms(1);
		In = GPIO_PORTF_DATA_R & 0x10;	// Read the switch
		if (In == 0x00) {								// Test if the switch is pressed
